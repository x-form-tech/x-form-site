...
## ⚠️ Целевая платформа — «1С-Битрикс: Управление сайтом» (решение 2026-07-11)

Этот Next.js-проект — **утверждаемый дизайн-прототип**, финальный сайт будет
перенесён на БУС (репутация интегратора, лиды нативно в Битрикс24 CRM, контент —
в инфоблоках). Следствия для работы здесь:

- Вкладываемся только в **переносимое**: вёрстку/CSS (Tailwind компилируется в
  обычный CSS), клиентский JS без React-специфики (темы, меню), контент и тексты.
- **Не** вкладываемся в Next-специфичную технику: Prisma/PostgreSQL и своя админка
  отменены (их заменят инфоблоки), `/api/lead` остаётся заглушкой (в БУС лиды
  пойдут нативно; временно можно REST-вебхук в Битрикс24, но без фанатизма).
- Интерактив писать максимально просто (ванильный JS-паттерн внутри React-обёртки),
  чтобы порт в PHP-шаблоны был дешёвым.
...
Файловый контент — **осознанное финальное решение для этого прототипа**: при
переносе на БУС массивы `lib/*` один-в-один становятся инфоблоками (услуги, кейсы,
статьи — готовая структура полей), поэтому никакой БД/админки здесь не будет.
Все места с временными данными помечены комментарием `TODO` — реальные цифры,
